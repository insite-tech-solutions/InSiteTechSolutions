...
const getOrganizedSections = (serviceTable: ServiceTable | undefined) => {
  ...
  Object.entries(serviceTable)
    ...
    .forEach(([key, value]) => {
      const sectionEntry: SectionEntry = [key, value as SectionEntryValue];
      if (
        key === "projectType" ||
...
export default function PriceCalculator({
  ...
  const costs = useMemo(() => calculateCosts(calculatorState, serviceTable), [calculatorState, serviceTable])

  const organizedSections = useMemo(() => getOrganizedSections(serviceTable), [serviceTable]);
