      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row gap-6 md:gap-8">
          {featuredProjects.map((project) => (
            <div
              key={project.id}
              className="group relative rounded-xl overflow-hidden bg-white shadow-sm transition-all duration-300 hover:shadow-md flex-1"
            ...
            >
              <div className="relative aspect-[3/4] w-full overflow-hidden">
                ...
                <div
                  className={cn(
                    "absolute bottom-0 left-0 right-0 bg-gradient-to-t from-blue-900/90 to-blue-900/70 backdrop-blur-sm transition-all duration-300 ease-in-out p-6",
                    hoveredProject === project.id ? "h-2/3" : "h-1/6",
                  )}
                >
                  <div className="h-full flex flex-col justify-between">
                    <h3 className="text-xl md:text-2xl font-semibold text-white mb-2">{project.title}</h3>

                    <div
                      className={cn(
                        "transition-opacity duration-300 flex flex-col justify-between h-full",
                        hoveredProject === project.id ? "opacity-100" : "opacity-0",
                      )}
                    >
                      <p className="text-gray-100 mb-4 line-clamp-3">{project.description}</p>

                      <div className="mt-auto">
                        <div className="flex flex-wrap gap-2 mb-4">
                          {project.technologies.slice(0, 3).map((tech) => (
                            <Badge key={tech} variant="secondary" className="bg-white/20 text-white hover:bg-white/30">
                              {tech}
                            </Badge>
                          ))}
